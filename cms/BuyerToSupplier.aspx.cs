...
public partial class cms_BuyerToSupplier : System.Web.UI.Page
{
    ...
    private bool page_right_check()
    {
        ...
        foreach (string item in GlobalArrays.returnarray())
        {
            ...
            if (item.Trim().CompareTo("BuyerToSupplier.aspx") == 0)
                result = true;
    ...
    protected void Filling_Gridview()
    {
        try
        {
            ...
            datast = new DataSet();
            datast = accessclass.GetDataSet("GetBuyerToSupplier", CommandType.StoredProcedure, sqlParams);
